...
Reading the configured font works for:

| Terminal                                            | Read from                                                                                     |
|-----------------------------------------------------|-----------------------------------------------------------------------------------------------|
| JetBrains IDEs (PhpStorm, IntelliJ, …)              | the IDE profile's `terminal-font.xml`; primary *and* secondary family, since JediTerm falls back to the secondary for glyphs the primary lacks |
| VS Code and forks (Cursor, VSCodium, Windsurf)      | `terminal.integrated.fontFamily`, falling back to `editor.fontFamily`; workspace settings win over user settings |
| macOS Terminal.app                                  | the `NSKeyedArchiver` blob in its profile                                                     |

Anywhere else, `auto` yields words; set `CLAUDE_STATUSLINE_ICONS=1` if your terminal does use a patched font. The result is cached for `CLAUDE_STATUSLINE_FONT_TTL` seconds, so after changing your terminal font either wait it out or `rm -rf ~/.claude/statusline-cache`.
